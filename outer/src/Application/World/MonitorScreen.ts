...
export default class MonitorScreen extends EventEmitter {
    ...
    constructor() {
        ...
        this.position = new THREE.Vector3(207, 1917, 190);
    ...
    setupDebug() {
        ...
        const folder = this.debug.ui.addFolder('Screen Position');
        const pos = { x: this.position.x, y: this.position.y, z: this.position.z };
        const rot = { x: this.rotation.x * THREE.MathUtils.RAD2DEG, y: this.rotation.y * THREE.MathUtils.RAD2DEG, z: this.rotation.z * THREE.MathUtils.RAD2DEG };
        const size = { w: SCREEN_SIZE.w, h: SCREEN_SIZE.h };

        folder.add(pos, 'x', -2000, 2000, 1).name('pos X').onChange((v: number) => { this.position.x = v; });
        folder.add(pos, 'y', -2000, 2000, 1).name('pos Y').onChange((v: number) => { this.position.y = v; });
        folder.add(pos, 'z', -2000, 2000, 1).name('pos Z').onChange((v: number) => { this.position.z = v; });
        folder.add(rot, 'x', -180, 180, 0.1).name('rot X (deg)').onChange((v: number) => { this.rotation.x = v * THREE.MathUtils.DEG2RAD; });
        folder.add(rot, 'y', -180, 180, 0.1).name('rot Y (deg)').onChange((v: number) => { this.rotation.y = v * THREE.MathUtils.DEG2RAD; });
        folder.add(rot, 'z', -180, 180, 0.1).name('rot Z (deg)').onChange((v: number) => { this.rotation.z = v * THREE.MathUtils.DEG2RAD; });
        folder.add(size, 'w', 100, 2000, 1).name('width');
        folder.add(size, 'h', 100, 2000, 1).name('height');
        folder.close();
    }
    ...
    createIframe() {
        ...
        iframe.title = 'JamieOS';
        // DOSBox games need pointer lock for mouse-look and fullscreen support
        iframe.allow = 'pointer-lock; fullscreen; autoplay; gamepad';
    ...
    createCssPlane(element: HTMLElement) {
        ...
        object.scale.set(SCREEN_SCALE, SCREEN_SCALE, SCREEN_SCALE);
    ...
    addTextureLayer(
        ...
        mesh.position.copy(
            this.offsetPosition(this.position, new THREE.Vector3(0, 0, offset * SCREEN_SCALE))
        );

        // Copy rotation and scale
        mesh.rotation.copy(this.rotation);
        mesh.scale.set(SCREEN_SCALE, SCREEN_SCALE, SCREEN_SCALE);
    ...
    createEnclosingPlanes(maxOffset: number) {
        const sw = this.screenSize.width * SCREEN_SCALE;
        const sh = this.screenSize.height * SCREEN_SCALE;
        const so = maxOffset * SCREEN_SCALE;

        const planes = {
            left: {
                size: new THREE.Vector2(so, sh),
                position: this.offsetPosition(
                    this.position,
                    new THREE.Vector3(-sw / 2, 0, so / 2)
                ),
            ...
            top: {
                size: new THREE.Vector2(sw, so),
                position: this.offsetPosition(
                    this.position,
                    new THREE.Vector3(0, sh / 2, so / 2)
                ),
    ...
    createPerspectiveDimmer(maxOffset: number) {
        ...
        mesh.position.copy(
            this.offsetPosition(
                this.position,
                new THREE.Vector3(0, 0, (maxOffset - 5) * SCREEN_SCALE)
            )
        );
